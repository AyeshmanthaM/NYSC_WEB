...
const Footer = () => {
  ...
  const getSVGFile = () => {
    return isDark 
      ? '/assets/svg/Yfooter_dark.svg' // Dark mode SVG
      : '/assets/svg/Yfooter_light.svg'; // Light mode SVG
  };
      ...
      {/* Top Decorative SVG Wave */}
        <div className="relative top-0 left-0 right-0 w-full h-auto z-0">
          <img
            src={getSVGFile()}
            alt=""
            className="w-full h-auto object-cover object-top transform scale-x-100"
          />
        </div>

      {/* content */}
      <div className={`relative -mt-2 ${getThemeColor('background.footer', isDark)}`}> 
