...
async function main() {
    ...
    const frontendLocalesPath = path_1.default.join(__dirname, '../../frontend/src/locales');
    const languages = ['en', 'si', 'ta'];
    let totalTranslations = 0;
    for (const language of languages) {
        for (const namespace of namespaces) {
            const filePath = path_1.default.join(frontendLocalesPath, language, `${namespace.name}.json`);
            try {
